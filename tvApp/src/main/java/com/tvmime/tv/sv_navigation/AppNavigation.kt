...
import com.tvmime.tv.sv_ui.screens.dashboard.DashboardScreen
import com.tvmime.tv.sv_ui.screens.multiview.MultiViewScreen
import com.tvmime.tv.sv_ui.screens.home.HomeScreen
import com.tvmime.tv.sv_ui.screens.movies.MoviesScreen
import com.tvmime.tv.sv_ui.screens.player.PlayerScreen
import com.tvmime.tv.sv_ui.screens.plugins.PluginsScreen
import com.tvmime.tv.sv_ui.screens.provider.ProviderSetupScreen
import com.tvmime.tv.sv_ui.screens.series.SeriesScreen
import com.tvmime.tv.sv_ui.screens.vod.VodScreen
import com.tvmime.tv.sv_ui.screens.settings.SettingsScreen
import com.tvmime.tv.sv_ui.screens.welcome.WelcomeScreen
import com.tvmime.tv.sv_ui.screens.downloads.DownloadsScreen
import com.tvmime.tv.MainActivity
...
@Composable
fun AppNavigation(viewModel: StreamVaultNavViewModel) {
    val navController = rememberNavController()
    val currentBackStackEntry = navController.currentBackStackEntryAsState().value
    val activeProvider = viewModel.providerRepository.getActiveProvider()
    ...
    LaunchedEffect(activeProvider?.id) {
        ...
        lastSplitCatalogType = viewModel.preferencesRepository
            .getLastSplitCatalogType(providerId)
            .first()
        loadedSplitPreferenceProviderId = providerId
    }
    val externalNavigationRequest = viewModel.externalNavigationRequestFlow.collectAsStateWithLifecycle().value
    val topLevelDestinations = viewModel.preferencesRepository.appTopLevelDestinations
        .collectAsStateWithLifecycle(initialValue = AppTopLevelDestination.defaultOrder)
        .value
    val appLandingDestination = viewModel.preferencesRepository.appLandingDestination
        .collectAsStateWithLifecycle(initialValue = null)
        .value
    ...
    val startupPlayerRequest by produceState<PlayerNavigationRequest?>(
        ...
        value = if (landing != null && requiresResolvedStartupTarget(landing)) {
            resolveStartupPlayerRequest(viewModel, landing)
        } else {
            null
        }
    }
        ...
        when (val request = externalNavigationRequest) {
            is ExternalNavigationRequest.Player -> {
                if (navController.navigateToExternalPlayer(request.request)) {
                    viewModel.clearExternalNavigationRequest()
                }
            }
    ...
    fun tabNavigate(route: String) {
        ...
        if (
            ...
            navigationScope.launch {
                viewModel.preferencesRepository.setLastSplitCatalogType(provider.id, selectedType)
            }
        }
...
// ... (rest of file)
        ...
        composable(
            ...
            com.tvmime.tv.sv_ui.screens.epg.FullEpgScreen(
        ...
        composable(
            ...
            com.tvmime.tv.sv_ui.screens.settings.parental.ParentalControlGroupScreen(
                currentRoute = Routes.SETTINGS,
                onNavigate = { route -> tabNavigate(route) },
                onBack = { navController.popBackStack() }
            )
        ...
        composable(
            ...
            com.tvmime.tv.sv_ui.screens.search.SearchScreen(
        ...
        composable(
            ...
            com.tvmime.tv.sv_ui.screens.movies.MovieDetailScreen(
                onPlay = { movie ->
                    navController.navigateToPlayer(
        ...
        composable(
            ...
            com.tvmime.tv.sv_ui.screens.series.SeriesDetailScreen(
...
private suspend fun resolveStartupPlayerRequest(
    viewModel: StreamVaultNavViewModel,
    landingDestination: AppLandingDestination
): PlayerNavigationRequest? = when (landingDestination) {
    AppLandingDestination.FIRST_FAVORITE_LIVE -> resolveFirstFavoriteStartupTarget(viewModel)
    AppLandingDestination.LAST_WATCHED_LIVE -> resolveLastWatchedStartupTarget(viewModel)
    else -> null
}
...
private suspend fun resolveFirstFavoriteStartupTarget(
    ...
    if (!viewModel.preferencesRepository.showFavoritesCategory.first()) return null
    val context = resolveLiveStartupContext(viewModel) ?: return null
    val favorites = when (context) {
        is LiveStartupContext.Provider -> viewModel.favoriteRepository.getFavorites(context.providerId, ContentType.LIVE).first()
        is LiveStartupContext.Combined -> viewModel.favoriteRepository.getFavorites(context.providerIds, ContentType.LIVE).first()
...
private suspend fun resolveLastWatchedStartupTarget(
    ...
    val recentHistory = when (context) {
        is LiveStartupContext.Provider -> viewModel.playbackHistoryRepository.getRecentlyWatchedByProvider(context.providerId, limit = 24).first()
        is LiveStartupContext.Combined -> viewModel.playbackHistoryRepository.getRecentlyWatchedByProviders(context.providerIds.toSet(), limit = 24).first()
    }
    return resolveStartupChannelTarget(
        mainActivity = viewModel,
...
private suspend fun resolveStartupChannelTarget(
    ...
    val hiddenChannelIdsByProvider = sourceContext.providerIds.associateWith { providerId ->
        viewModel.preferencesRepository.getHiddenChannelIds(providerId).first()
    }
    for (channelId in channelIds.distinct()) {
        val channel = viewModel.channelRepository.getChannel(channelId) ?: continue
...
private suspend fun resolveLiveStartupContext(
    ...
    return when (val activeSource = viewModel.combinedM3uRepository.getActiveLiveSource().first()) {
        ...
        is ActiveLiveSource.CombinedM3uSource -> {
            val providerIds = viewModel.combinedM3uRepository.getProfile(activeSource.profileId)
                ?.members
                .orEmpty()
                .filter { it.enabled }
                .map { it.providerId }
        ...
        null -> {
            viewModel.providerRepository.getActiveProvider().first()?.id?.let { providerId ->
